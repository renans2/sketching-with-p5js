import type { BouncingLinesProps } from "../types/sketches-props";
import type { ZustandStore } from "../types/ZustandStore";
import { getCanvasSize } from "../utils/canvas-parent";
import type p5 from "p5";
import { initMethods } from "../utils/define-store-methods";

export const sketch = (p: p5, store: ZustandStore<BouncingLinesProps>) => {
  const { p5Remove, vars, unsubscribe } = initMethods<BouncingLinesProps>(
    "bouncing-lines",
    p,
    store,
  );
  ...
  p.draw = () => {
    ...
    const hue = (p.frameCount * vars.colorSpeed) % 360;
